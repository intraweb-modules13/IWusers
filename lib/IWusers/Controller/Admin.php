...
function iw_users_admin_main($args)
{
	...
	if ($inici == null) {
		$inici = ModUtil::func('iw_main', 'user', 'userInitVar',
		                    array('sv' => $sv,
								  'module' => 'iw_users',
								  'name' => 'inici',
								  'default' => '0',
		...
		ModUtil::func('iw_main', 'user', 'userSetVar',
		           array('sv' => $sv,
						 'module' => 'iw_users',
						 'name' => 'inici',
						 'value' => $inici,
	...
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	if ($filtre == null) {
		$filtre = ModUtil::func('iw_main', 'user', 'userInitVar',
		                     array('sv' => $sv,
								   'module' => 'iw_users',
								   'name' => 'filtre',
								   'default' => '0',
	...
	if ($campfiltre == null  || $campfiltre == '') {
		$campfiltre = ModUtil::func('iw_main', 'user', 'userInitVar',
		                         array('sv' => $sv,
									   'module' => 'iw_users',
									   'name' => 'campfiltre',
									   'default' => 'l',
	...
	if ($numitems == null || $numitems == '') {
		$numitems = ModUtil::func('iw_main', 'user', 'userInitVar',
		                       array('sv' => $sv,
									 'module' => 'iw_users',
									 'name' => 'numitems',
									 'default' => '20',
	...
	$allUsers = ModUtil::apiFunc('iw_users', 'user', 'getAllUsers');
	// Get all users info
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	$usersUname = ModUtil::func('iw_main', 'user', 'getAllUsersInfo',
	                         array('info' => 'l',
								   'sv' => $sv));															
	...
	foreach ($notExist as $key => $value) {
		ModUtil::apiFunc('iw_users', 'admin', 'create',
		              array('uid' => $key));
	}
	// Count the users for the criteria
	$usersNumber = ModUtil::apiFunc('iw_users', 'user', 'countUsers',
	                             array('campfiltre' => $campfiltre,
																		'filtre' => $filtre));
	// Get all users needed
	$users = ModUtil::apiFunc('iw_users', 'user', 'getAll',
	                       array('campfiltre' => $campfiltre,
								 'filtre' => $filtre,
								 'inici' => $inici,
								 'numitems' => $numitems));
	...
	// Get all users mails
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	$usersMail = ModUtil::func('iw_main', 'user', 'getAllUsersInfo',
	                        array('info' => 'e',
								  'sv' => $sv,
								  'list' => $usersList));
	...
	foreach ($users as $user) {
		$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
		$groups = ModUtil::func('iw_main', 'user', 'getAllUserGroups',
		                     array('sv' => $sv,
								   'uid' => $user['uid']));															
		$userGroups = array();
	...
	$pager = ModUtil::func('iw_users', 'admin', 'pager',
	                    array('inici' => $inici,
							  'rpp' => $numitems,
	...
	return $view->fetch('iw_users_admin_main.htm');
}
...
function iw_users_admin_module()
{
	...
	$module = ModUtil::func('iw_main', 'user', 'module_info', array('module_name' => 'iw_users', 'type' => 'admin'));

	$view->assign('module', $module);
	return $view->fetch('iw_users_admin_module.htm');
}
...
function iw_users_admin_edit($args)
{
	...
	if ($userId == null) {
		...
		return System::redirect(ModUtil::url('iw_users', 'admin', 'main'));
	}

	// get users registers
	$users = ModUtil::apiFunc('iw_users', 'user', 'get', array('multi' => $userId));
	if ($users == false) {
	...
	// Get all users info
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	$usersNames = ModUtil::func('iw_main', 'user', 'getAllUsersInfo', array('info' => 'l',
																		'sv' => $sv,
																		'list' => $usersList));

	foreach ($users as $user) {
		$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
		$photo = ModUtil::func('iw_main', 'user', 'getUserPicture', array('uname' => $usersNames[$user['uid']],
																		'sv' => $sv));
	...
	if (!file_exists(ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'usersPictureFolder')) || ModUtil::getVar('iw_main', 'usersPictureFolder') == '') {
		$canChangeAvatar = false;
	} else {
		if (!is_writeable(ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'usersPictureFolder'))) {
			$canChangeAvatar = false;
		}
	}

	//Check if the temp folder exists
	if (!file_exists(ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'tempFolder')) || ModUtil::getVar('iw_main', 'tempFolder') == '') {
		$canChangeAvatar = false;
	} else {
		if (!is_writeable(ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'tempFolder'))) {
			$canChangeAvatar = false;
		}
	}
	...
	return $view->fetch('iw_users_admin_edit.htm');
}
...
function iw_users_admin_update($args)
{
	...
	if (!SecurityUtil::confirmAuthKey()) {
		return LogUtil::registerAuthidError (ModUtil::url('iw_users', 'admin', 'main'));
	}

	$error = false;
	if (!ModUtil::apiFunc('iw_users', 'admin', 'updateUser', array('uid' => $uid,
																'nom' => $nom,
																'cognom1' => $cognom1,
																'cognom2' => $cognom2))) {
	...
	$folder = ModUtil::getVar('iw_main', 'tempFolder');
	...
	foreach ($uid as $u) {
		if ($deleteAvatar[$u] != 1) {
			...
			if ($fileName != '' && ($file_extension == 'png' || $file_extension == 'gif' || $file_extension == 'jpg')) {
				...
				$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
				$update = ModUtil::func('iw_main', 'user', 'updateFile', array('sv' => $sv,
																			'folder' => $folder,
			...
			if ($nom_fitxer != '') {
				for($i=0; $i<2; $i++) {
					...
					$imgSource = ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'tempFolder').'/'.$nom_fitxer;
					$imgDest = ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'usersPictureFolder').'/'.$userFileName;
					
					//if success $errorMsg = ''
					$errorMsg = ModUtil::func('iw_main', 'user', 'thumb', array('imgSource' => $imgSource,
																			'imgDest' => $imgDest,
																			'new_width' => $new_width,
																			'deleteOthers' => 1));
				}
				...
				unlink(ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'tempFolder').'/'.$nom_fitxer);
			}
		} else {
			ModUtil::func('iw_main', 'user', 'deleteAvatar', array('avatarName' => $usersNames[$u],
																'extensions' => array('jpg','png','gif')));
			ModUtil::func('iw_main', 'user', 'deleteAvatar', array('avatarName' => $usersNames[$u].'_s',
																'extensions' => array('jpg','png','gif')));
		}
	}
...
function iw_users_admin_new()
{
	...
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	$groups = ModUtil::func('iw_main', 'user', 'getAllGroups', array('sv' => $sv));
	...
	return $view->fetch('iw_users_admin_new.htm');
}
...
function iw_users_admin_create($args)
{
	...
	// Get all users uname
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	$usersUname = ModUtil::func('iw_main', 'user', 'getAllUsersInfo', array('info' => 'l',
																		'sv' => $sv,
																		'list' => $uname.'$$'));
	...
	$user = ModUtil::apiFunc('iw_users', 'admin', 'createUser', array('uname' => $uname,
																	'pass' => $pass,
																	'nom' => $nom,
																	'cognom1' => $cognom1,
																	'cognom2' => $cognom2));
	...
	if ($nom_fitxer != '') {
		for($i=0; $i<2; $i++) {
			...
			//if success $errorMsg = ''
			$errorMsg = ModUtil::func('iw_main', 'user', 'thumb',
	...
	$userData = ModUtil::apiFunc('iw_users', 'admin', 'create',
	                          array('uid' => $user,
									'nom' => $nom,
									'cognom1' => $cognom1,
									'cognom2' => $cognom2));
	...
	$userInitGroup = ModUtil::apiFunc('iw_users', 'admin', 'addUserToGroup',
	                               array('uid' => $user,
										 'gid' => $group));
...
function iw_users_admin_delete($args)
{
	...
	if (!$confirmation) {
		// get users registers
		$users = ModUtil::apiFunc('iw_users', 'user', 'get',
		                       array('multi' => $userId));
		if ($users == false) {
			LogUtil::registerError (__('No users found', $dom));
			return System::redirect(ModUtil::url('iw_users', 'admin', 'main'));
		...
		return $view->fetch('iw_users_admin_delete.htm');
	}
	...
	foreach ($uid as $u) {
		...
		ModUtil::func('iw_main', 'user', 'deleteAvatar',
	...
	$deleteIWUser = ModUtil::apiFunc('iw_users','admin','deleteIWUser',
	                              array('uid' => $uid));
	if (!$deleteIWUser) {
	...
	if ($deleteIWUser) {
		$deleteUserGroups = ModUtil::apiFunc('iw_users','admin','deleteUserGroups',
		                                  array('uid' => $uid));
		if (!$deleteUserGroups) {
	...
	if ($deleteUserGroups) {
		$deleteUsers = ModUtil::apiFunc('iw_users','admin','deleteUsers',
		                             array('uid' => $uid));
		if (!$deleteUsers) {
...
function iw_users_admin_pager($args)
{
	...
	return $view->fetch('iw_users_admin_pager.htm');
}
...
function iw_users_admin_export($args)
{
	...
	$file = ModUtil::getVar('iw_main', 'documentRoot') . '/' . ModUtil::getVar('iw_main', 'tempFolder') . '/export' . date('dmY') . '.csv';
	$f = fopen($file, 'w');
	...
	switch ($pas) {
		...
		case 2:
			...
			if ($extensio_fitxer != 'csv' || $len_fitxer == 0) { 
				...
				if (!move_uploaded_file($_FILES['fitxer']['tmp_name'], ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'tempFolder').$nom_fitxer)) { 
			...
			$fp = fopen(ModUtil::getVar('iw_main', 'documentRoot').'/'.ModUtil::getVar('iw_main', 'tempFolder').$nom_fitxer,'r');
			...
			$lid = ModUtil::apiFunc('iw_users', 'admin', 'truncateTables');
			if ($lid == false) {
			...
			$lid = ModUtil::apiFunc('iw_users', 'admin', 'copyTables');
			if ($lid == false) {
				$view->assign('error2', true);
				$pas = 0;
				break;
				...
				foreach ($registres[1] as $registre) {
					...
					switch ($i % 5) {
						...
						case 4; //cognom2
							...
							$lid = ModUtil::apiFunc('iw_users', 'admin', 'createImport', array('accio' => 0,
																							'id' => $id,
																							'nom' => $nom,
																							'cognom1' => $cognom1,
																							'cognom2' => $cognom2));
			...
			$primer_filtre = ModUtil::apiFunc('iw_users', 'admin', 'primer_filtre');
		...
		case 3:
			...
			$conflictes = ModUtil::apiFunc('iw_users','admin','conflictes');
			if ($conflictes == false) {
					$error = true;
					$error_text = 1;
			} else {
			...
			switch($subpas) {
				case 1:
					...
					if (!empty($tria)) {
						...
						foreach ($tria as $tria1) {
							...
							$accio = ModUtil::apiFunc('iw_users', 'admin', 'modificaaccio',
							                       array('taula' => 1,
														 'valor' => $valor1,
														 'suid' => $suids[$i]));
							$accio = ModUtil::apiFunc('iw_users', 'admin', 'modificaaccio',
					...
					$usuaris = ModUtil::apiFunc('iw_users', 'admin', 'get_gestio',
					                         array('subpas' => 1,
												   'inici' => 0,
												   'numitems' => 20));
					...
					if (!$no_hi_ha) {
						...
						foreach ($usuaris as $usuari) {
							...
							$quins_grups = '';
							$grups = ModUtil::apiFunc('iw_users', 'user', 'quins_grups', array('uid' => $usuari['uid']));
					...
					if ($conflictes['diferents'] > 0) {
						...
						$sortida->linebreak(2);
						$sortida->URL(DataUtil::formatForDisplay(ModUtil::url('iw_users', 'admin', 'import',array('pas' => 3,
																		'subpas' => 0))),__('Back to the menu of options', $dom));
					} else {
						//Enviem a l'usuari al menÃº de gestiÃ³ de les dades
						System::redirect(ModUtil::url('iw_users', 'admin', 'import',array('pas' => 3,
														'subpas' => 0)));
						$sortida->linebreak(2);
						$sortida->URL(DataUtil::formatForDisplay(ModUtil::url('iw_users', 'admin', 'import', array('pas' => 3,
																											'subpas' => 0))),__('Back to the menu of options', $dom));
				...
				case 2:
						...
						foreach ($tria as $tria1) {
							switch($tria[$i]) {
								case 1:
									...
									$valor = 3;
									$modificasaga = ModUtil::apiFunc('iw_users', 'admin', 'modificasaga', array('valor' => 3,
																											'suid' => $suids[$i],
																											'login' => $login[$i],
																											'email' => $email[$i],
																											'grup' => $group,
									...
									break;
								case 2:
									$modificasaga = ModUtil::apiFunc('iw_users', 'admin', 'modificasaga', array('valor' => 4,
																											'suid' => $suids[$i],
																											'uid' => $associa[$i]));

									$accio = ModUtil::apiFunc('iw_users', 'admin', 'modificaaccio', array('taula' => 2,
																										'valor' => (-4),
																										'uid' => $associa[$i],
																										'suid' => $suids[$i]));
									break;
								case 3:
									$accio = ModUtil::apiFunc('iw_users', 'admin', 'modificaaccio', array('taula' => 1,
																										'valor' => (-3),
																										'suid' => $suids[$i]));
									break;
							}
					...
					$usuaris = ModUtil::apiFunc('iw_users', 'admin', 'get_gestio', array('subpas' => 2,
																						'inici' => 0,
																						'numitems' => 20));
					if ($usuaris == false) {
						$sortida->linebreak(1);
					...
					if (!$no_hi_ha) {
						...
						$registres = ModUtil::apiFunc('iw_users', 'admin', 'get_gestio', array('subpas' => 3,
																							'inici' => 0,
																							'numitems' => 9999999999));
						...
						$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
						$grups_MS = ModUtil::func('iw_main', 'user', 'getAllGroups', array('sv' => $sv));		
						...
						foreach ($usuaris as $usuari) {
							...
							if ($usuari['login'] == "") {
								$login = ModUtil::func('iw_users', 'admin', 'posanom', array('nom' => $usuari['noms'],
							...
							if ($usuari['contrasenya'] == "") {
								...
								$pass = ModUtil::func('iw_users', 'admin', 'posanom', array('nom' => '',
																						'cognom1' => $pass));
							} else {
				...
				case 3:
					...
					$sortida->linebreak(1);
					//Buquem els usuaris que tenen un 1 a les dues taules
					$usuaris = ModUtil::apiFunc('iw_users', 'admin', 'get_gestio', array('subpas' => 3,
																						'inici' => 0,
																						'numitems' => 20));
					if ($usuaris == false) {
						$sortida->linebreak(1);
				...
				case 4:
					...
					if (!empty($submit)) {
						...
						foreach ($suids as $a) {
							...
							if ($suids[$i] != '') {
								$registre = ModUtil::apiFunc('iw_users', 'admin', 'get_dades', array('taula' => 1,
																									'suid' => $suids[$i]));
																									
								if ($registre == false) {
									SessionUtil::setVar('errormsg', __('An error occurred at the time of carrying out the orders', $dom));
									System::redirect(ModUtil::url('iw_users', 'admin', 'import', array('pas' => 3,
																								'subpas' => 4)));
									return false;
								}
								switch($registre['accio']) {
									case 2:
										...
										$lid = ModUtil::apiFunc('iw_users', 'admin', 'modifica', array('id' => $registre['id'],
																									'nom' => $registre['nom'],
																									'cognom1' => $registre['cognom1'],
																									'cognom2' => $registre['cognom2'],
																									'origen' => 'saga'));
										if ($lid) {
											ModUtil::apiFunc('iw_users','admin','esborra_registre',array('taula' => 1,
																										'suid'=>$suids[$i]));
										} else {
											SessionUtil::setVar('errormsg', __('An error occurred at the time of carrying out the orders', $dom));
										}
									...
									case 3:
										...
										if (empty($registre['login'])) {
											...
											return $view->fetch('iw_users_admin_import.htm');
										}
										...
										$lid = ModUtil::apiFunc('iw_users', 'admin', 'createUser', array('uname' => $registre['login'],
										...
										if ($lid == false) {
											SessionUtil::setVar('statusmsg', __('There was an error to register a new user.', $dom));
											System::redirect(ModUtil::url('iw_users', 'admin', 'import', array('pas' => 3,
																										'subpas' => 4)));
											return false;
										}
										...
										//Entrem les dades de l'usuari
									   	$lid1 = ModUtil::apiFunc('iw_users', 'admin', 'create', array('id' => $registre['id'],
																									'uid' => $lid,
																									'nom' => $registre['nom'],
																									'cognom1' => $registre['cognom1'],
																									'cognom2' => $registre['cognom2']));
										...
										if ($lid1 == false) {
											SessionUtil::setVar('statusmsg', __('We have created the user, but there was an error in entering the personal information.', $dom));
											System::redirect(ModUtil::url('iw_users', 'admin', 'import', array('pas' => 3,
																										'subpas' => 4)));
											return false;
										}
										...
										//Posem l'usuari dins del grup escollit
									   	$lid2 = ModUtil::apiFunc('iw_users', 'admin', 'addUserToGroup', array('uid' => $lid,
																											'gid' => $registre['grup']));

										if ($lid2 == false) {
											SessionUtil::setVar('statusmsg', __('We have created the user, but there was a failure to assign it to the group', $dom));
											System::redirect(ModUtil::url('iw_users', 'admin', 'import', array('pas'=>3,
																										'subpas'=>4)));
											return false;
										}
										...
										if ($lid1) {
											...
											ModUtil::apiFunc('iw_users', 'admin', 'modificaaccio', array('taula' => 1,
																										'valor' => -7,
																										'suid' => $suids[$i]));
										} else {
											SessionUtil::setVar('errormsg', __('An error occurred at the time of carrying out the orders', $dom));
							...
							if ($suidi[$i]!='') {
								$registre=ModUtil::apiFunc('iw_users', 'admin', 'get_dades', array('taula' => 2,
																								'suid' => $suidi[$i]));
								if ($registre==false) {
									SessionUtil::setVar('errormsg', __('An error occurred at the time of carrying out the orders', $dom));
									System::redirect(ModUtil::url('iw_users', 'admin', 'import', array('pas' => 3,
								...
								switch($registre['accio']) {
									case 5:
										...
										ModUtil::apiFunc('iw_users', 'admin', 'deleteUser', array('uid' => $registre['uid']));
										//Esborrem l'usuari de la taula de grups de PN
										ModUtil::apiFunc('iw_users', 'admin', 'deleteUserFromGroups', array('uid' => $registre['uid']));
										break;
									default:
										break;
								}
								ModUtil::apiFunc('iw_users', 'admin', 'esborra_registre', array('taula' => 2,
																							'suid' => $suidi[$i]));
							}
							$i++;
						}
					...
					//Buquem els usuaris que tenen un 1 a les dues taules
					$usuaris = ModUtil::apiFunc('iw_users', 'admin', 'get_ordres', array('inici' => 0,
																						'numitems' => 20));
					if ($usuaris == false) {
						$sortida->linebreak(1);
					...
					if (!$no_hi_ha) {
						...
						foreach ($usuaris as $usuari) {
							...
							$sortida->Tablecolend();
							$sortida->Tablecolstart();
							$sortida->URL(DataUtil::formatForDisplay(ModUtil::url('iw_users', 'admin', 'import', array('quins' => $quins,
																												'taula' => $taula,
																												'pas' => 3,
					...
					if ($conflictes['ordres'] > 0) {
							...
							$sortida->linebreak(2);
							$sortida->URL(DataUtil::formatForDisplay(ModUtil::url('iw_users','admin','import',array('pas'=>3,'subpas'=>0))),__('Back to the menu of options', $dom));
					} else {
						//Enviem a l'usuari al menï¿œ de gestiï¿œ de les dades
						System::redirect(ModUtil::url('iw_users', 'admin', 'import', array('pas' => 3,
																					'subpas' => 0)));
				...
				case 5: //Reseteja totes les modificacions portades a terme
						ModUtil::apiFunc('iw_users', 'admin', 'fesreset', array('quins' => $quins));
						System::redirect(ModUtil::url('iw_users', 'admin', 'import', array('pas' => 3,
																					'subpas' => 0)));
						break;
				case 6: //Reseteja una modificaciï¿œ
						ModUtil::apiFunc('iw_users', 'admin', 'fesresetuna', array('quins' => $quins,
													'taula' => $taula));
						System::redirect(ModUtil::url('iw_users', 'admin', 'import', array('pas' => 3,
														'subpas' => 4)));
						$sortida->linebreak(2);
				...
				case 7: //Marca els registres per no fer acciï¿œ sobre d'ells
					ModUtil::apiFunc('iw_users','admin','capaccio',array('quins'=>$quins));
					System::redirect(ModUtil::url('iw_users','admin','import',array('pas'=>3)));
					break;					
				default;
		...
		case 4:
			...
			$usuaris = ModUtil::apiFunc('iw_users', 'admin', 'get_gestio', array('subpas' => 4,
												'inici' => 0,
			...
			if ($usuaris == false) {
				...
				$sortida->Tableend();
				$sortida->linebreak(1);
				$sortida->URL(DataUtil::formatForDisplay(ModUtil::url('iw_users', 'admin', 'fitxer', array('pas' => 4))), __('To create a txt file with the data, click on the link', $dom));
			}
			$sortida->LineBreak(2);
			//Esborrem el contingut de les taules auxiliars de la importaciï¿œ i retornem a la taula d'usuaris
			$sortida->URL(DataUtil::formatForDisplay(ModUtil::url('iw_users', 'admin', 'final_import')), __('The process ends', $dom));
			break;
	}
...
function iw_users_admin_fitxer($args)
{
	...
	$fitxer = ModUtil::getVar('iw_main', 'documentRoot') . '/' . ModUtil::getVar('iw_main', 'tempFolder') . '/usuaris'.date('dmY').'.txt';
...
function iw_users_admin_final_import($args)
{
	...
	ModUtil::apiFunc('iw_users','admin','buida_taules');
	return System::redirect(ModUtil::url('iw_users', 'admin', 'main'));
}
...
function iw_users_admin_editLogin($args)
{
	...
	// Get all users info
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	$usersFullNames = ModUtil::func('iw_main', 'user', 'getAllUsersInfo',
	                             array('info' => 'ncc',
									   'sv' => $sv,
									   'list' => $usersList));
	// Get all users info
	...
	// Get all users email
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	$usersMails = ModUtil::func('iw_main', 'user', 'getAllUsersInfo', array('info' => 'e',
																		'sv' => $sv,
																		'list' => $usersList));
	...
	return $view->fetch('iw_users_admin_editLogin.htm');
}
...
function iw_users_admin_updateLogin($args)
{
	...
	foreach ($uid as $u) {
	    ...
	    //Es crida la funciÃ³ API amb les dades extretes del formulari
   		$lid = ModUtil::apiFunc('iw_users', 'admin', 'modificacontrasenya',
   		                     array('userId' => $u,
...
function iw_users_admin_config()
{
	...
	$friendsSystemAvailable = ModUtil::getVar('iw_users', 'friendsSystemAvailable');
	$invisibleGroupsInList = ModUtil::getVar('iw_users', 'invisibleGroupsInList');
	// Create output object
	$view = Zikula_View::getInstance('iw_users',false);
	$sv = ModUtil::func('iw_main', 'user', 'genSecurityValue');
	$groups = ModUtil::func('iw_main', 'user', 'getAllGroups',
	                     array('sv' => $sv));										
	foreach ($groups as $group) {
		$checked = false;	
	...
	return $view->fetch('iw_users_admin_config.htm');
}
...
function iw_users_admin_updateConf($args)
{
	...
	ModUtil::setVar('iw_users','friendsSystemAvailable', $friendsSystemAvailable);
	ModUtil::setVar('iw_users','invisibleGroupsInList', $groupsString);
