...
function iw_users_adminapi_copyTables()
{
	...
	$users = ModUtil::apiFunc('iw_users', 'user', 'getAll', array('inici' => 0,
																'numitems' => 1000000000));
	if($users == false){
		return false;
	}
...
function iw_users_adminapi_modifica($args)
{
	...
	if($origen=='saga'){
		$link = ModUtil::apiFunc('iw_users', 'user', 'get',array('id' => $id));
	}else{
		$link = ModUtil::apiFunc('iw_users', 'user', 'get',array('uid' => $uid));
	}
	//Comprovem que la consulta anterior ha tornat amb resultats
	if ($link == false){
		SessionUtil::setVar('errormsg', __('Could not find the user on which do the action.', $dom));
		return false;
	}
	//Comprovacions de seguretat
	if (!SecurityUtil::checkPermission*(0, 'iw_users::', "::", ACCESS_ADMIN)) {
		SessionUtil::setVar('errormsg', __('Not authorized to manage users.', $dom));
		return false;
	}
	//Establim connexiï¿œ amb la base de dades
	$dbconn =& DBConnectionStack::getConnection*(true);
	...
	if($origen == 'saga'){
		if($login != ""){
			$canvi_login = ModUtil::apiFunc('iw_users', 'admin', 'modificacontrasenya', array('uid' => $link['uid'],
																							'nomusuari' => $login));
		}		$sql = "UPDATE
				$t
				SET
...
function iw_users_adminapi_modificacontrasenya($args)
{
	...
	//Comprovem que els valors han arribat
	if (!isset($userId)) {
		return SessionUtil::setVar('errormsg', __('Error! Could not do what you wanted. Please check your input.', $dom));
	}
	//Cridem la funciï¿œ get de l'API que ens retornarï¿œ les dades de l'anotaciï¿œ
	$link = ModUtil::apiFunc('iw_users', 'admin', 'get_dades', array('suid' => $userId));
	//Comprovem que la consulta anterior ha tornat amb resultats
	if ($link == false) {
		return SessionUtil::setVar('errormsg', __('Could not find the user on which do the action.', $dom));
	}
	//Establim connexiï¿œ amb la base de dades
	$dbconn =& DBConnectionStack::getConnection*(true);
	$pntable =& DBUtil::getTables();
...
function iw_users_adminapi_fesresetuna($args)
{
	...
	$registre = ModUtil::apiFunc('iw_users', 'admin', 'get_dades', array('suid' => $quins,
																		'taula' => $taula));
	if ($registre == false){
		SessionUtil::setVar('statusmsg', __('Could not find the user on which do the action.', $dom));
		return false;
	}
	//Establim connexiï¿œ amb la base de dades
	$dbconn =& DBConnectionStack::getConnection*(true);
